...
Route-level metadata (summaries, tags, operation IDs) is supplied per method in `openapi.ts` next to each route. The docs UI defaults to Swagger UI, with `scalarDocs()` and `redocDocs()` as alternatives. See [OpenAPI Generation](/docs/openapi/generation) and [Swagger UI](/docs/openapi/swagger-ui).
