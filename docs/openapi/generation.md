...
An interactive docs UI is served from the spec. The default provider is Scalar; `swaggerDocs()` and `redocDocs()` are the built-in alternatives, chosen in the OpenAPI config (`openapi.config.ts` in dev, the `openapi` option in production builds).
